...
interface RestaurantClientProps {
  restaurant: typeof locations[0] | undefined;
}

export default function RestaurantClient({ restaurant: loc }: RestaurantClientProps) {
  // Create images array - if single image, duplicate 3 times for testing
  const images = loc?.image 
    ? (Array.isArray(loc.image) ? loc.image : [loc.image, loc.image, loc.image])
    : [];
    ...
    <main className="min-h-screen bg-white pb-20">
      ...
      <div className="relative h-72 w-full">
        ...
        <div 
          ...
          className="h-full w-full overflow-x-auto snap-x snap-mandatory scrollbar-hide flex"
          style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
        >
          {images.map((image, index) => (
            <div key={index} className="min-w-full h-full snap-start relative flex-shrink-0">
              <img 
                src={image} 
                alt={`${loc.name} - Image ${index + 1}`} 
        ...
        <div className="absolute top-0 w-full p-4 flex justify-between items-center z-10">
          <Link href="/" className="bg-black/40 p-2 rounded-full text-white hover:bg-black/50 transition-all">
          ...
          <button 
            onClick={handleShare}
            className="bg-black/40 p-2 rounded-full text-white hover:bg-black/50 transition-all active:scale-90"
          >
        ...
        <div className="absolute bottom-0 w-full p-6 text-white">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="px-2 py-1 bg-black/40 text-white text-[10px] font-bold uppercase tracking-wider rounded-md border border-white/30">
                {loc.price}
            </span>
          </div>
